...
from ..utils import email_from_dict, email_to_dict
...
class DjangoQBackend(BaseEmailBackend):
    ...
    def send_messages(self, email_messages):
        ...
        for email_message in email_messages:
            ...
            async_task('backend.email.backends.django_q.send_message', email_message)
            num_sent += 1
        return num_sent
...
def send_message(email_message):
    ...
    try:
        if isinstance(email_message, dict):
            email_message = email_from_dict(email_message)
